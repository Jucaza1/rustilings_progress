...
fn return_str_or_slice(b:bool,a:&str) -> Cow<str>{
    if b{
        a.to_owned().into()
    } else{
        a.into()
...
fn main() {
    ...
    match return_str_or_slice(false,"hello"){
        Cow::Owned(s) => print!("this is owned -> {}",s),
        Cow::Borrowed(s) => print!("this is borrowed -> {}",s),
